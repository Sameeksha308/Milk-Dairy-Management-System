...
<body>
  ...
  <?php if (!isset($_POST['farmer_login'])): ?>
  <div class="form-box">
    <form method="post" action="farmer.php">
      ...
      <div class="input-group">
        <button type="submit" class="btn" name="farmer_login">Login</button>
      </div>
    </form>
  </div>
  <?php endif; ?>
  ...
  if (isset($_POST['farmer_login'])) {
      ...
      $f_password = mysqli_real_escape_string($db, $_POST['f_password']);

      $query = "SELECT * FROM farmer WHERE username='$f_username' AND password='$f_password'";
      ...
      if (mysqli_num_rows($result) == 1) {
          ...
          echo "<p><strong>Membership Date:</strong> {$row['membership_date']}</p>";
